...
export async function findBlockingOverlap(input: {
  ...
  /** Roster additions share their teacher by definition; still enforce the student's diary. */
  ignoreTeacherConflicts?: boolean;
}): Promise<OverlapClash | null> {
  const { id, teacherId, studentId, date, hours, groupId, bookingBatchId, ignoreTeacherConflicts } = input;
  ...
  for (const c of candidates) {
    ...
    const sameGroup =
      ...
      (!!bookingBatchId && c.bookingBatchId === bookingBatchId);

    if (sameStudent || (!ignoreTeacherConflicts && sameTeacher && !sameGroup)) {
      return {
