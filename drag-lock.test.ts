...
const scheduler: DragPolicy = { canDrag: true, lockConflicted: false, graceDays: 7 };
const viewer: DragPolicy = { canDrag: false, lockConflicted: false, graceDays: 7 };
...
describe("lockReasonFor — what may be moved", () => {
  ...
  it("refuses an exam or assessment, whatever else is configured", () => {
    for (const sessionType of ["EXAM", "ASSESSMENT"] as const) {
      ...
      expect(
        lockReasonFor(subject({ sessionType }), { canDrag: true, lockConflicted: false, graceDays: 7 }),
      ).toBe("STRICT_TYPE");
    }
  });

  it("lets a taught lesson be corrected inside the grace window", () => {
    // Records get fixed after the fact. A week is the centre's default.
    for (const status of ["COMPLETED", "CHECKED_IN"]) {
      expect(lockReasonFor(subject({ status, daysOld: 0 }), scheduler)).toBeNull();
      expect(lockReasonFor(subject({ status, daysOld: 7 }), scheduler)).toBeNull();
      expect(lockReasonFor(subject({ status, daysOld: 8 }), scheduler)).toBe("TOO_OLD");
    }
  });
  ...
  it("does NOT believe a future lesson that claims to be finished", () => {
    ...
    expect(lockReasonFor(subject({ status: "COMPLETED", daysOld: -7 }), scheduler)).toBeNull();
  });

  it("closes the books once the window has passed, whatever else is true", () => {
    // TOO_OLD outranks every other reason: nothing about a closed day is
    // still a scheduling question.
    expect(lockReasonFor(subject({ daysOld: 400, sessionType: "EXAM" }), scheduler)).toBe("TOO_OLD");
    expect(lockReasonFor(subject({ daysOld: 400 }), { ...scheduler, graceDays: 0 })).toBe("TOO_OLD");
  });

  it("still refuses a cancelled lesson inside the window", () => {
    for (const status of ["CANCELLED", "NO_SHOW"]) {
      expect(lockReasonFor(subject({ status, daysOld: 0 }), scheduler)).toBe("ALREADY_HAPPENED");
    }
  });
...
describe("lockReasonFor — the conflicted case is the centre's choice", () => {
  ...
  it("allows dragging a clashing lesson by default — that is how you fix it", () => {
    expect(lockReasonFor(clashing, { canDrag: true, lockConflicted: false, graceDays: 7 })).toBeNull();
  });

  it("locks it once the centre asks for that", () => {
    expect(lockReasonFor(clashing, { canDrag: true, lockConflicted: true, graceDays: 7 })).toBe("CONFLICTED");
  });
});
