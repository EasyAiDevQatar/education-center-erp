...
describe("requiredGapMin — distance, not a guess", () => {
  // The real pair: جميلة's home to دانة's home, 15.4 km apart. Booked 15
  // minutes apart, it passed the flat rule in silence and produced a journey
  // needing 54 minutes that no driver could make.
  const near = { lat: 25.2854, lng: 51.531 };
  const far = { lat: 25.4106, lng: 51.4894 };
  const home = (startMin: number, endMin: number, at: { lat: number; lng: number }) => ({
    startMin,
    endMin,
    location: "HOME",
    at,
  });
  /** Stand-in for the engine's travelMinutes: 3.5 min per 0.01° of latitude. */
  const travel = (a: { lat: number }, b: { lat: number }) =>
    Math.ceil(Math.abs(a.lat - b.lat) * 350);

  it("demands the journey's real length once it can measure it", () => {
    const need = requiredGapMin(home(0, 60, near), home(75, 135, far), DEFAULT_SPACING, travel);
    expect(need).toBeGreaterThan(DEFAULT_SPACING.homeBufferMin);
    expect(need).toBe(44);
  });

  it("keeps the buffer as a floor for two houses on the same street", () => {
    // Dismissing one student and settling the next is real time whatever the
    // map says, so the rule never drops below the configured buffer.
    const need = requiredGapMin(home(0, 60, near), home(75, 135, near), DEFAULT_SPACING, travel);
    expect(need).toBe(DEFAULT_SPACING.homeBufferMin);
  });

  it("falls back to the flat buffer when a pin is missing", () => {
    const unpinned = { startMin: 75, endMin: 135, location: "HOME" };
    expect(requiredGapMin(home(0, 60, near), unpinned, DEFAULT_SPACING, travel)).toBe(
      DEFAULT_SPACING.homeBufferMin,
    );
  });

  it("still lets two centre lessons run back to back", () => {
    const centre = (s: number, e: number) => ({ startMin: s, endMin: e, location: "CENTER", at: far });
    expect(requiredGapMin(centre(0, 60), centre(60, 120), DEFAULT_SPACING, travel)).toBe(0);
  });

  it("now REPORTS the case that used to pass in silence", () => {
    const p = spacingProblem(home(0, 60, near), home(75, 135, far), DEFAULT_SPACING, travel);
    expect(p).not.toBeNull();
    expect(p!.kind).toBe("TOO_TIGHT");
    expect(p!.shortfallMin).toBe(29); // needs 44, has 15
  });
});
